from flask import Blueprint, flash, redirect, render_template, request, session, url_for

from auth import login_required
from db import get_db_connection
from utils import (
    delete_uploaded_image,
    get_categories,
    get_cities,
    get_countries,
    save_uploaded_image,
)

attractions_bp = Blueprint("attractions", __name__, url_prefix="/content-admin/attractions")

PAGE_SIZE = 20

STATUS_CHOICES = ("active", "hidden", "pending")


def _parse_form(form_data):
    form = {
        "category_id": form_data.get("category_id", "").strip() or None,
        "name": form_data.get("name", "").strip(),
        "country_id": form_data.get("country_id", "").strip(),
        "city_id": form_data.get("city_id", "").strip(),
        "address": form_data.get("address", "").strip() or None,
        "opening_hours": form_data.get("opening_hours", "").strip() or None,
        "ticket_price": form_data.get("ticket_price", "").strip(),
        "suggested_duration_minutes": form_data.get("suggested_duration_minutes", "").strip(),
        "description": form_data.get("description", "").strip() or None,
        "website_url": form_data.get("website_url", "").strip() or None,
        "is_popular": form_data.get("is_popular") == "on",
        "status": form_data.get("status", "active").strip(),
        "remove_image": form_data.get("remove_image") == "on",
    }

    errors = []

    if not form["name"]:
        errors.append("請輸入景點名稱")

    if not form["country_id"]:
        errors.append("請選擇國家")

    if not form["city_id"]:
        errors.append("請選擇城市")

    if form["ticket_price"] == "":
        form["ticket_price"] = 0
    else:
        try:
            form["ticket_price"] = float(form["ticket_price"])
            if form["ticket_price"] < 0:
                raise ValueError
        except ValueError:
            errors.append("票價必須是不小於 0 的數字")
            form["ticket_price"] = 0

    if form["suggested_duration_minutes"] == "":
        form["suggested_duration_minutes"] = None
    else:
        try:
            form["suggested_duration_minutes"] = int(form["suggested_duration_minutes"])
            if form["suggested_duration_minutes"] < 0:
                raise ValueError
        except ValueError:
            errors.append("建議停留時間必須是不小於 0 的整數")
            form["suggested_duration_minutes"] = None

    if form["status"] not in STATUS_CHOICES:
        form["status"] = "active"

    return form, errors


def _load_options(cursor):
    return {
        "countries": get_countries(cursor),
        "cities": get_cities(cursor),
        "categories": get_categories(cursor, "attraction"),
    }


@attractions_bp.route("/")
@login_required("content_admin")
def list_attractions():
    keyword = request.args.get("keyword", "").strip()
    country_id = request.args.get("country_id", "").strip()
    city_id = request.args.get("city_id", "").strip()
    category_id = request.args.get("category_id", "").strip()
    status = request.args.get("status", "").strip()
    page = request.args.get("page", "1")
    page = int(page) if page.isdigit() and int(page) > 0 else 1

    connection = get_db_connection()

    if connection is None:
        flash("資料庫連線失敗", "error")
        return render_template(
            "content_admin/attractions/list.html",
            attractions=[], countries=[], cities=[], categories=[],
            keyword=keyword, country_id=country_id, city_id=city_id,
            category_id=category_id, status=status, page=1, total_pages=1, total=0
        )

    cursor = connection.cursor(dictionary=True)

    try:
        conditions = []
        params = []

        if keyword:
            conditions.append("(a.name LIKE %s OR a.address LIKE %s)")
            params.extend([f"%{keyword}%", f"%{keyword}%"])

        if country_id:
            conditions.append("a.country_id = %s")
            params.append(country_id)

        if city_id:
            conditions.append("a.city_id = %s")
            params.append(city_id)

        if category_id:
            conditions.append("a.category_id = %s")
            params.append(category_id)

        if status:
            conditions.append("a.status = %s")
            params.append(status)

        where_clause = ("WHERE " + " AND ".join(conditions)) if conditions else ""

        cursor.execute(
            f"SELECT COUNT(*) AS total FROM attractions a {where_clause}",
            params
        )
        total = cursor.fetchone()["total"]

        total_pages = max(1, (total + PAGE_SIZE - 1) // PAGE_SIZE)
        page = min(page, total_pages)
        offset = (page - 1) * PAGE_SIZE

        cursor.execute(
            f"""
            SELECT a.attraction_id, a.name, a.address, a.ticket_price,
                   a.suggested_duration_minutes, a.image_path, a.status,
                   a.is_popular, a.ai_verified_at,
                   cat.category_name, co.name AS country_name, ci.name AS city_name
            FROM attractions a
            LEFT JOIN categories cat ON cat.category_id = a.category_id
            JOIN countries co ON co.country_id = a.country_id
            JOIN cities ci ON ci.city_id = a.city_id
            {where_clause}
            ORDER BY a.attraction_id DESC
            LIMIT %s OFFSET %s
            """,
            params + [PAGE_SIZE, offset]
        )
        attractions = cursor.fetchall()

        options = _load_options(cursor)

    finally:
        cursor.close()
        connection.close()

    return render_template(
        "content_admin/attractions/list.html",
        attractions=attractions,
        countries=options["countries"],
        cities=options["cities"],
        categories=options["categories"],
        keyword=keyword,
        country_id=country_id,
        city_id=city_id,
        category_id=category_id,
        status=status,
        page=page,
        total_pages=total_pages,
        total=total,
    )


@attractions_bp.route("/new", methods=["GET", "POST"])
@login_required("content_admin")
def create_attraction():
    connection = get_db_connection()

    if connection is None:
        flash("資料庫連線失敗", "error")
        return redirect(url_for("attractions.list_attractions"))

    cursor = connection.cursor(dictionary=True)

    try:
        options = _load_options(cursor)

        if request.method == "POST":
            form, errors = _parse_form(request.form)

            if not errors:
                try:
                    image_path = save_uploaded_image(request.files.get("image"), "attractions")
                except ValueError as error:
                    errors.append(str(error))
                    image_path = None

            if errors:
                for message in errors:
                    flash(message, "error")
                return render_template(
                    "content_admin/attractions/form.html",
                    mode="create", attraction=form, **options
                )

            try:
                cursor.execute(
                    """
                    INSERT INTO attractions
                    (category_id, name, country_id, city_id, address, opening_hours,
                     ticket_price, suggested_duration_minutes, description, website_url,
                     image_path, is_popular, status, created_by)
                    VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                    """,
                    (
                        form["category_id"], form["name"], form["country_id"], form["city_id"],
                        form["address"], form["opening_hours"], form["ticket_price"],
                        form["suggested_duration_minutes"], form["description"],
                        form["website_url"], image_path, form["is_popular"], form["status"],
                        session["user_id"]
                    )
                )
                connection.commit()
                flash("景點新增成功", "success")
            except Exception as error:
                connection.rollback()
                print("新增景點失敗：", error)
                flash("新增景點失敗", "error")

            return redirect(url_for("attractions.list_attractions"))

        return render_template(
            "content_admin/attractions/form.html",
            mode="create", attraction=None, **options
        )

    finally:
        cursor.close()
        connection.close()


@attractions_bp.route("/<int:attraction_id>/edit", methods=["GET", "POST"])
@login_required("content_admin")
def edit_attraction(attraction_id):
    connection = get_db_connection()

    if connection is None:
        flash("資料庫連線失敗", "error")
        return redirect(url_for("attractions.list_attractions"))

    cursor = connection.cursor(dictionary=True)

    try:
        options = _load_options(cursor)

        cursor.execute("SELECT * FROM attractions WHERE attraction_id = %s", (attraction_id,))
        existing = cursor.fetchone()

        if existing is None:
            flash("找不到這筆景點資料", "error")
            return redirect(url_for("attractions.list_attractions"))

        if request.method == "POST":
            form, errors = _parse_form(request.form)
            image_path = existing["image_path"]

            new_file = request.files.get("image")
            if new_file and new_file.filename:
                try:
                    uploaded_path = save_uploaded_image(new_file, "attractions")
                    if uploaded_path:
                        delete_uploaded_image(existing["image_path"])
                        image_path = uploaded_path
                except ValueError as error:
                    errors.append(str(error))
            elif form["remove_image"]:
                delete_uploaded_image(existing["image_path"])
                image_path = None

            if errors:
                for message in errors:
                    flash(message, "error")
                form["attraction_id"] = attraction_id
                form["image_path"] = image_path
                return render_template(
                    "content_admin/attractions/form.html",
                    mode="edit", attraction=form, **options
                )

            try:
                cursor.execute(
                    """
                    UPDATE attractions
                    SET category_id = %s, name = %s, country_id = %s, city_id = %s,
                        address = %s, opening_hours = %s, ticket_price = %s,
                        suggested_duration_minutes = %s, description = %s,
                        website_url = %s, image_path = %s, is_popular = %s, status = %s
                    WHERE attraction_id = %s
                    """,
                    (
                        form["category_id"], form["name"], form["country_id"], form["city_id"],
                        form["address"], form["opening_hours"], form["ticket_price"],
                        form["suggested_duration_minutes"], form["description"],
                        form["website_url"], image_path, form["is_popular"], form["status"],
                        attraction_id
                    )
                )
                connection.commit()
                flash("景點資料已更新", "success")
            except Exception as error:
                connection.rollback()
                print("更新景點失敗：", error)
                flash("更新景點失敗", "error")

            return redirect(url_for("attractions.list_attractions"))

        return render_template(
            "content_admin/attractions/form.html",
            mode="edit", attraction=existing, **options
        )

    finally:
        cursor.close()
        connection.close()


@attractions_bp.route("/<int:attraction_id>/delete", methods=["POST"])
@login_required("content_admin")
def delete_attraction(attraction_id):
    connection = get_db_connection()

    if connection is None:
        flash("資料庫連線失敗", "error")
        return redirect(url_for("attractions.list_attractions"))

    cursor = connection.cursor(dictionary=True)

    try:
        cursor.execute(
            "SELECT image_path FROM attractions WHERE attraction_id = %s",
            (attraction_id,)
        )
        existing = cursor.fetchone()

        cursor.execute("DELETE FROM attractions WHERE attraction_id = %s", (attraction_id,))
        connection.commit()

        if existing:
            delete_uploaded_image(existing["image_path"])

        flash("景點已刪除", "success")

    except Exception as error:
        connection.rollback()
        print("刪除景點失敗：", error)
        flash("刪除景點失敗，請確認沒有其他資料仍在使用此景點", "error")

    finally:
        cursor.close()
        connection.close()

    return redirect(url_for("attractions.list_attractions", **request.args.to_dict()))
