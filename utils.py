...
from urllib.parse import parse_qs, urlparse  # 匯入網址解析工具，用來拆解 Google 圖片檢視頁網址
...
def save_image_from_url(url, subfolder):  # 定義函式：從網址下載圖片並存到 static/uploads/子資料夾，回傳相對路徑
    ...
    try:  # 嘗試連線下載圖片
        response = requests.get(  # 用串流模式下載，避免一次把整個檔案讀進記憶體
            url, timeout=10, stream=True, headers=IMAGE_DOWNLOAD_HEADERS  # 帶上瀏覽器表頭，避免被防盜連機制擋掉
        )
        response.raise_for_status()  # 如果 HTTP 狀態碼不是 2xx，直接拋出例外
