...
restaurants_bp = Blueprint("restaurants", __name__, url_prefix="/content-admin/restaurants")  # 建立餐廳管理藍圖，網址前綴 /content-admin/restaurants
...
def _parse_form(form_data):  # 定義內部函式：把表單資料解析成乾淨的欄位字典，並做基本驗證
    form = {  # 從表單中逐一取出欄位，並做去除空白等初步處理
        ...
        "name": form_data.get("name", "").strip(),  # 餐廳名稱
        ...
        "cuisine_type": form_data.get("cuisine_type", "").strip() or None,  # 料理類型，空字串轉成 None
        "price_level": form_data.get("price_level", "medium").strip(),  # 價位等級，預設中價位
        "opening_hours": form_data.get("opening_hours", "").strip() or None,  # 營業時間，空字串轉成 None
        "description": form_data.get("description", "").strip() or None,  # 餐廳描述，空字串轉成 None
    ...
    if form["price_level"] not in PRICE_LEVEL_CHOICES:  # 如果價位等級不在合法清單內(防止表單被竄改)
        form["price_level"] = "medium"  # 強制改回預設值
...
@restaurants_bp.route("/")  # 設定餐廳列表頁路由
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def list_restaurants():  # 定義餐廳列表頁函式
    ...
    try:  # 開始查詢資料
        ...
        cursor.execute(  # 查詢當頁的餐廳資料，並關聯分類、國家、城市名稱
            f"""
        ...
        restaurants = cursor.fetchall()  # 取出當頁餐廳清單
    ...
    return render_template(  # 渲染餐廳列表頁面
        "content_admin/restaurants/list.html",
        restaurants=restaurants,  # 當頁餐廳清單
...
@restaurants_bp.route("/new", methods=["GET", "POST"])  # 設定新增餐廳頁路由
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def create_restaurant():  # 定義新增餐廳函式
    connection = get_db_connection()  # 建立資料庫連線
            ...
            try:  # 嘗試寫入資料庫
                cursor.execute(  # 執行新增餐廳的 SQL
                    ...
                    (
                        ...
                        form["address"], form["cuisine_type"], form["price_level"],  # 地址、料理類型、價位
                        form["opening_hours"], form["description"], form["website_url"],  # 營業時間、描述、網站
                        image_path, form["status"], session["user_id"]  # 圖片路徑、狀態、建立者
                    )
                )
                log_action(  # 把這次新增動作寫入操作紀錄
                    cursor, "create_restaurant", "restaurant", cursor.lastrowid,
                    f"新增餐廳：{form['name']}"
                )
                connection.commit()  # 提交交易，正式寫入資料庫(餐廳資料與操作紀錄一起寫入)
                flash("餐廳新增成功", "success")  # 顯示成功訊息
            except Exception as error:  # 如果寫入過程發生例外
                connection.rollback()  # 回復交易
                print("新增餐廳失敗：", error)  # 在伺服器端印出錯誤內容
                flash("新增餐廳失敗", "error")  # 顯示錯誤提示

            return redirect(url_for("restaurants.list_restaurants"))  # 不論成功或失敗，都導回餐廳列表頁
...
@restaurants_bp.route("/<int:restaurant_id>/edit", methods=["GET", "POST"])  # 設定編輯餐廳頁路由，網址帶入餐廳 ID
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def edit_restaurant(restaurant_id):  # 定義編輯餐廳函式
    connection = get_db_connection()  # 建立資料庫連線
    ...
    try:  # 開始處理表單/資料庫操作
        ...
        if request.method == "POST":  # 如果是表單送出請求
            ...
            if new_file and new_file.filename:  # 如果使用者有選擇新圖片檔案
                try:  # 嘗試儲存新圖片
                    uploaded_path = save_uploaded_image(new_file, "restaurants")  # 儲存新圖片，取得新路徑
            ...
            if errors:  # 如果有任何驗證錯誤
                for message in errors:  # 逐一顯示每個錯誤訊息
                    flash(message, "error")
                form["restaurant_id"] = restaurant_id  # 把餐廳 ID 補回表單資料
            ...
            try:  # 嘗試更新資料庫
                cursor.execute(  # 執行更新餐廳的 SQL
                    ...
                    (
                        ...
                        form["address"], form["cuisine_type"], form["price_level"],  # 地址、料理類型、價位
                        form["opening_hours"], form["description"], form["website_url"],  # 營業時間、描述、網站
                        image_path, form["status"], restaurant_id  # 圖片路徑、狀態、WHERE 條件用的餐廳 ID
                    )
                )
                log_action(  # 把這次更新動作寫入操作紀錄
                    cursor, "update_restaurant", "restaurant", restaurant_id,
                    f"更新餐廳：{form['name']}"
                )
                connection.commit()  # 提交交易，正式更新資料庫(餐廳資料與操作紀錄一起寫入)
                flash("餐廳資料已更新", "success")  # 顯示成功訊息
            except Exception as error:  # 如果更新過程發生例外
                connection.rollback()  # 回復交易
                print("更新餐廳失敗：", error)  # 在伺服器端印出錯誤內容
                flash("更新餐廳失敗", "error")  # 顯示錯誤提示
...
@restaurants_bp.route("/<int:restaurant_id>/delete", methods=["POST"])  # 設定刪除餐廳的路由，網址帶入餐廳 ID，只允許 POST
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def delete_restaurant(restaurant_id):  # 定義刪除餐廳函式
    connection = get_db_connection()  # 建立資料庫連線
    ...
    try:  # 開始執行刪除
        ...
        if existing:  # 如果原本有查到這筆資料(代表確實刪除了某筆餐廳)
            log_action(  # 把這次刪除動作寫入操作紀錄
                cursor, "delete_restaurant", "restaurant", restaurant_id,
...
IMPORT_HEADER = [  # 定義餐廳 CSV 匯入需要的欄位順序，範本下載與匯入解析都用這份清單
    "name", "country", "city", "category", "address", "cuisine_type",
    "price_level", "opening_hours", "description", "website_url", "status"
]


@restaurants_bp.route("/import/template.csv")  # 設定下載餐廳匯入範本的路由
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def download_restaurant_template():  # 定義下載匯入範本函式
    example_row = [  # 準備一列範例資料，讓使用者知道每個欄位該怎麼填
        "鼎泰豐（信義店）", "台灣", "台北市", "日式料理", "台北市信義區松高路194號",
        "中式料理", "medium", "11:00-21:00", "知名小籠包餐廳", "https://www.dintaifung.com.tw",
        "active"
    ]
    return csv_response(  # 組成並回傳 CSV 下載回應
        "restaurants_import_template.csv",  # 下載檔名
        IMPORT_HEADER,  # CSV 表頭
        [example_row]  # CSV 範例資料列
    )


@restaurants_bp.route("/import", methods=["GET", "POST"])  # 設定餐廳 CSV 匯入頁路由
@login_required("content_admin")  # 限制只有內容管理員登入後才能存取
def import_restaurants():  # 定義餐廳 CSV 匯入函式
    if request.method == "GET":  # 如果是進頁面(還沒上傳檔案)
        return render_template("content_admin/restaurants/import.html")  # 顯示上傳表單頁面
    ...
    if not uploaded_file or not uploaded_file.filename:  # 如果沒有選擇檔案
        ...
        return redirect(url_for("restaurants.import_restaurants"))  # 導回匯入頁面
    ...
    try:  # 開始逐列處理匯入資料
        for row_number, row in enumerate(reader, start=2):  # 從第 2 列開始編號(第 1 列是表頭)
            name = (row.get("name") or "").strip()  # 取得餐廳名稱
            ...
            price_level = (row.get("price_level") or "medium").strip()  # 取得價位等級，預設中價位
            if price_level not in PRICE_LEVEL_CHOICES:  # 如果不是合法價位等級
                price_level = "medium"  # 修正為預設值
            ...
            try:  # 嘗試處理這一列的國家/城市/分類並寫入資料庫
                ...
                category_id = get_or_create_category(cursor, "restaurant", category_name)  # 取得(或新增)分類 ID
                ...
                cursor.execute(  # 執行新增餐廳的 SQL
                    ...
                    (
                        ...
                        (row.get("cuisine_type") or "").strip() or None,  # 料理類型
                        price_level,  # 價位等級
                        (row.get("opening_hours") or "").strip() or None,  # 營業時間
        ...
        log_action(  # 把這次批次匯入寫入操作紀錄
            cursor, "import_restaurants", "restaurant", None,
            f"CSV 匯入餐廳，成功 {success_count} 筆，失敗 {len(results) - success_count} 筆"
        )
    ...
    return render_template(  # 渲染匯入結果頁面
        "content_admin/restaurants/import_result.html",
